...
const getBotResponse = async (instruction, input = null, config = {}, fewShot = []) => {
  ...
  const generationConfig = { ...defaultConfig, ...config };
    ...
    // ✅ แล้วค่อย log ค่าทั้งหมด
    console.log("📤 Prompt ที่ส่ง:", prompt);
    console.log("📦 Config ที่ส่ง:", generationConfig);
    ...
    if (filteredMessage !== message) {
      ...
      if (intent && fewShotExamples[intent]) {
        ...
        if (!botResponse || botResponse.trim() === "") {
          return res.status(200).json({ reply: "ขออภัย บอทยังไม่มีคำตอบสำหรับคำถามนี้" });
        }
